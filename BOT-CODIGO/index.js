...
if (!db.users[sender]) {
    db.users[sender] = { name: pushName || 'Usuario', totalCommands: 0, favCommands: {} };
  } else {
    db.users[sender].name = pushName || db.users[sender].name;
    if (!db.users[sender].favCommands) db.users[sender].favCommands = {};
  }
...
async function handleTopGacha(sock, msg) {
  const entries = Object.entries(db.users).filter(([uid]) => {
    for (const gid of Object.keys(db.groups)) {
      if (db.groups[gid].harems?.[uid]?.length) return true;
    }
    return false;
  }).sort((a, b) => {
    const countA = Object.values(db.groups).reduce((sum, g) => sum + (g.harems?.[a[0]]?.length || 0), 0);
    const countB = Object.values(db.groups).reduce((sum, g) => sum + (g.harems?.[b[0]]?.length || 0), 0);
    return countB - countA;
  }).slice(0, 10);
  if (!entries.length) return sock.sendMessage(msg.key.remoteJid, { text: '◇ Sin datos.' }, { quoted: msg });
  let text = `✦ Top Gacha (global por personajes)\n\n`;
  for (const [uid, u] of entries) {
    const total = Object.values(db.groups).reduce((sum, g) => sum + (g.harems?.[uid]?.length || 0), 0);
    text += `✦ ${uid} » ${u.name}\n    ✤ Personajes \`${total}\`\n`;
  }
  await sock.sendMessage(msg.key.remoteJid, { text }, { quoted: msg });
}
...
// ━━━━━━━ TOP / BALTOP / BALTOPGP ━━━━━━━
async function handleTop(sock, msg, args) {
  const page = parseInt(args[1]) || 1;
  const perPage = 10;
  const all = Object.entries(db.users).filter(([, u]) => u.totalCommands > 0)
    .sort((a, b) => (b[1].level || 1) - (a[1].level || 1) || (b[1].totalExp || 0) - (a[1].totalExp || 0));
  ...
  let text = `『✦』Top Usuarios con más Nivel (global)\n\n`;
  slice.forEach(([id, u], i) => {
    text += `✦ ${(page-1)*perPage + i + 1} » ${u.name}\n    ✤ XP \`${u.totalExp?.toLocaleString() || 0}\` ➭ LVL \`${u.level || 1}\`\n`;
  });
  text += `\n✦ Página \`${page}/${pages}\``;
  await sock.sendMessage(msg.key.remoteJid, { text }, { quoted: msg });
}
...
async function start() {
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    if (db.botOff && !(text?.startsWith('/bot on'))) return;

    user.totalCommands = (user.totalCommands || 0) + 1;
    if (!user.favCommands) user.favCommands = {};
    user.favCommands[command] = (user.favCommands[command] || 0) + 1;
    if (
db.botOff &&
!(command === 'bot' && args[1] === 'on')
) return;
switch (command) {
